...
size_t size_extract_between(wchar_t *str,  wchar_t *p1,  wchar_t *p2)
{
  const wchar_t *i1 = wcsstr(str, p1);
  if(i1 != NULL)
  {
    const size_t pl1 = wcslen(p1);
    const wchar_t *i2 = wcsstr(i1 + pl1, p2);
    if(p2 != NULL)
    {
     /* Found both markers, extract text. */
     const size_t mlen = i2 - (i1 + pl1);
     wchar_t *ret = malloc(mlen + 1);
     if(ret != NULL)
     {
